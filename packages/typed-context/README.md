...
`typed-context` wraps Effection's context utilities so yielded operations keep track of which context values are required and which providers satisfy them.

## Install

```bash
pnpm add typed-context effection
```

## Usage

`typed-context` supports two integration styles:

1. Import from `typed-context` as a drop-in replacement for the relevant
   Effection APIs.
2. Keep importing from `effection` and add the `typed-context` ambient type
   augmentation.

### Drop-in replacement

Import the typed APIs directly from `typed-context`:
...
```ts
/// <reference types="typed-context/types" />
```
...
- `strip(operation)`
  ...
  drop-in API from `typed-context`; with the `effection` module augmentation,
